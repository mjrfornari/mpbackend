...
async function gerapdf(ped, user) {
    return new Promise (async (resolve)=>{
            ...
            // console.log(sql)
                ...
                db.query(sql, async function(err, result) {
                    ...
                    itepedido.forEach((element, id) => {
                        element.VALOR_COM_DESCONTO = Math.round((((element.VALOR * ((100-element.DESCONTO1)/100))*((100-element.DESCONTO2)/100))*((100-element.DESCONTO3)/100)) * 100)/100
                        element.VALOR_IPI = Math.round((((element.IPI || 0)/100)*element.VALOR_COM_DESCONTO*element.QUANTIDADE)*100)/100
                        element.VALORITENS = Math.round((element.VALOR_COM_DESCONTO*element.QUANTIDADE)*100)/100  
                    })
                    ...
                    var detail = function (x, r) {
                        x.band([
                            ...
                            {data: r.DESCRICAOPRO, width: 220, align: 1, fontSize: 9},
                            {data: r.QUANTIDADE, width: 35, align: 3, fontSize: 9},
                            {data: (r.VALOR).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }).replace(",","*").replace(".",",").replace("*","."), width: 65, align: 3, fontSize: 9},
                            {data: (r.VALOR_COM_DESCONTO).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }).replace(",","*").replace(".",",").replace("*","."), width: 65, align: 3, fontSize: 9},
                            {data: (r.VALORITENS).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }).replace(",","*").replace(".",",").replace("*","."), width: 70, align: 3, fontSize: 9},
                            {data: (r.IPI || 0).toFixed(2).replace(",","*").replace(".",",").replace("*","."), width: 30, align: 3, fontSize: 9},
                            {data: (r.VALOR_IPI || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }).replace(",","*").replace(".",",").replace("*","."), width: 55, align: 3, fontSize: 9},
                            {data: (r.PERC_STICMS || 0).toFixed(2).replace(",","*").replace(".",",").replace("*","."), width: 50, align: 3, fontSize: 9},
                            ...
                            {data: ((r.VALOR_COM_DESCONTO*r.QUANTIDADE)+(r.VALOR_STICMS || 0)+(r.VALOR_IPI || 0)).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }).replace(",","*").replace(".",",").replace("*","."), width: 90, align: 3, fontSize: 9},                        
                        ], {x: 20});
                    };
        ...
        var proposalHeader = function (x, r) {
            ...
            x.band([{data: pedido.ORCAMENTO==='S' ? 'Nº Orçamento:' : 'Nº Pedido:', width: 150}, {data: pedido[0].NUMWEB || pedido[0].NUMWEB, width: 150, align: "right", fontSize: 9}], {
                x: 500,
                y: 58
            });
            x.band([{data: 'Data:', width: 150}, {data: dataISOtoDefault(pedido[0].DATA.toISOString()), width: 150,align: "right", fontSize: 9}], {x: 500});
            x.band([{data: 'Vendedor(a):', width: 150}, {data: pedido[0].VENDEDOR, width: 150,align: "right", fontSize: 9}], {x: 500});
            x.band([{data: 'Cliente:', width: 150}, {data: pedido[0].RAZAO_SOCIAL, width: 150,align: "right", fontSize: 9}], {x: 500});
            x.band([{data: 'Condição de Pagamento:', width: 150}, {data: pedido[0].NOMECPG, width: 150,align: "right", fontSize: 9}], {x: 500});
            x.newline();
            x.newline();
            ...
            x.band([
                ...
                {data: 'Descrição', width: 220, align: 1, fontSize: 9},
                {data: 'Qtd', width: 35, align: 1, fontSize: 9},
                {data: 'Unit. s/ Desc', width: 65, align: 1, fontSize: 9},
                {data: 'Unit. c/ Desc', width: 65, align: 1, fontSize: 9},
                {data: 'Valor Total', width: 70, align: 1, fontSize: 9},
                {data: '% IPI', width: 30, align: 1, fontSize: 9},
                {data: 'Valor IPI', width: 55, align: 1, fontSize: 9},
                {data: '% ST ICMS', width: 50, align: 1, fontSize: 9},
...
app.get('/api/startSync',  function (req, res, next) {
    try {
        if (!isEmpty(req.query)) {
            date = new Date ()
            name = date.toISOString().replace(/\./g, '').replace(/:/g,'').replace(/-/g,'').replace(/Z/g,'')
            console.log(name)
            sync = {
                qty: {
                    create: 0,
            ...
            let versaoSistema = req.query.versao.replace(/\./g, '')
            fs.writeFileSync('sync_log/sync_'+req.query.user+'_v'+versaoSistema+'_'+name+'.json', arquivo, 'utf8');
            res.json('sync_'+req.query.user+'_v'+versaoSistema+'_'+name)
            console.log('Sync stared: '+'sync_'+req.query.user+'_v'+versaoSistema+'_'+name)
        } else {
            res.status(404).end();
        }
    } 
...
app.get('/api/identifySync',  function (req, res, next) {
    try {
        if (!isEmpty(req.query)) {
            ...
            fs.readFile('sync_log/'+req.query.arquivo+'.json', 'utf8', (err, data) => {
                if (err) throw err;
                ...
                fs.writeFileSync('sync_log/'+req.query.arquivo+'.json', arquivo, 'utf8');
            })
        } else {
            res.status(404).end();
        }
...
async function commitTransaction(transaction, db, res){
    return new Promise (async (resolve) => {
        console.log('commit')
        try {
            console.log('commit')
...
function sendTransaction(sync, transaction, res){
    return new Promise (async (resolve) => {
        let count = 0
        if (sync.length === 0){
            resolve('Done')
        ...
        for(let i of sync){
            i = i.split(',ITENS,').join(',').split(',ID,').join(',').split(',ID=,').join(',').split(',ID=UNDEFINED,').join(',').split(',MOSTRAMODAL=,').join(',')
            ...
            fs.readFile('sync_log/'+req.query.arquivo+'.json', 'utf8', (err, data) => {    
                ...
                Firebird.attach(optionsfb, function(err, db) {
                    ...
                    db.transaction(Firebird.ISOLATION_READ_UNCOMMITTED, async function(err, transaction) {
                        ...
                        await sendTransaction(sync.create, transaction).then((resolve)=>{
                            sync.steps.push('Create:')
                            sync.steps.push(resolve)
                        })
                        console.log('Created')
                        // .then(async (resolve) => {
                        await sendTransaction(sync.update, transaction).then((resolve)=>{
                            sync.steps.push('Update:')
                            sync.steps.push(resolve)
                        })
                        console.log('Updated')
                        // .then(async (resolve)=>{
                        await sendTransaction(sync.delete, transaction).then((resolve)=>{
                            sync.steps.push('Delete:')
                            sync.steps.push(resolve)
                        })
                        console.log('Deleted')
                        // .then(async (resolve)=>{
                        commitTransaction(transaction, db, res).then(async (resolve)=>{
                            if (resolve === 'Commited!') {
                                // fs.unlinkSync('sync_log/sync_'+req.query.user+'_'+name+'.json',function(err){
                                        // if(err) return console.log(err);
                                console.log('file deleted successfully');
                                sync.steps.push('Commited')
                                arquivo = JSON.stringify(sync)
                                fs.writeFileSync('sync_log/'+req.query.arquivo+'.json', arquivo, 'utf8');

                                // });
                            } else {
                                sync.steps.push('Not Commited')
                                arquivo = JSON.stringify(sync)
                                // fs.writeFileSync('sync_log/sync_'+req.query.user+'_'+name+'.json', arquivo, 'utf8');
                                fs.writeFileSync('sync_log/'+req.query.arquivo+'.json', arquivo, 'utf8');
                            }
